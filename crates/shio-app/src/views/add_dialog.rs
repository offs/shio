...
mod tests {
    ...
    fn effective_destination_appends_subfolder_when_enabled() {
        ...
        let result = effective_destination(&base.display().to_string(), true, "archive");

        assert_eq!(result, base.join("archive").display().to_string());
    }

    #[test]
    fn effective_destination_sanitizes_subfolder_when_enabled() {
        let base = Path::new("downloads");
        let result = effective_destination(&base.display().to_string(), true, " bad/name ");

        assert_eq!(result, base.join("bad_name").display().to_string());
    }

    #[test]
    fn effective_destination_omits_empty_subfolder() {
        let result = effective_destination("downloads", true, " ");
