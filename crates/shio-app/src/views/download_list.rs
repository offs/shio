...
pub(crate) fn view<'a>(
    ...
    let headers = column_headers(options.sort_col, options.sort_dir, options.widths, p);
    ...
    for (dl, indices) in downloads {
        ...
        let row_el = download_row::view(
            ...
            download_row::NameDisplayOptions {
                ...
                name_width: options.widths.name,
            },
            options.widths,
            p,
        );
        ...
        let draggable: Element<'a, Message> = if dl.pinned {
            container(row_el).id(widget_id).into()
        } else {
            iced_drop::droppable(row_el)
                .id(widget_id)
...
fn column_headers<'a>(
    ...
    let header = |label: &str, col: SortCol, width: f32| -> Element<'a, Message> {
    ...
    let row = iced::widget::row![
        Space::new().width(28),
        header("name", SortCol::Name, widths.name),
        resize_handle(DownloadColumn::Name, p),
        header("size", SortCol::Size, widths.size),
        resize_handle(DownloadColumn::Size, p),
        header("progress", SortCol::Progress, widths.progress),
        resize_handle(DownloadColumn::Progress, p),
        header("speed", SortCol::Speed, widths.speed),
        resize_handle(DownloadColumn::Speed, p),
        header("eta", SortCol::Eta, widths.eta),
