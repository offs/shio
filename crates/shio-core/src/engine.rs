...
impl DownloadEngine {
    ...
    fn resolve_magnet_preview(
        ...
        const PREVIEW_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(120);
    ...
    fn spawn_progress_task(
        ...
        tokio::spawn(async move {
            ...
            while let Some(progress) = rx.recv().await {
...

                {
                    ...
                    if let Some(d) = downloads.get_mut(&id) {
                        ...
                        if let Some(snapshot) = progress.torrent_snapshot.as_ref() {
                            if let Some(torrent) = d.torrent_mut() {
                ...
                if should_flush {
                    if let Some(db_writer) = db_writer.as_ref() {
                        if let Err(e) = db_writer.try_send(DbWrite::Progress {
                            id,
                    ...
                    {
                        if let Some(db_writer) = db_writer.as_ref() {
                            if let Err(e) = db_writer.try_send(DbWrite::TorrentRuntime {
                                id,
                                uploaded: *uploaded,
                                ratio: *ratio,
...
fn validate_download_paths(download: &Download) -> crate::Result<()> {
    ...
    if let Some(http) = download.http() {
        if let Some(subfolder) = http.subfolder.as_deref() {
